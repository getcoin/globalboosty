...
We recommend that everybody running prior versions of globalboostd/GlobalBoost-Qt
...
in your terminal, then install the globalboost-qt package:
  sudo apt-get install globalboost-qt
...
Code:
/Applications/GlobalBoost-Qt
(on Mac) or
Code:
globalboostd/globalboost-qt
(on Linux).
...
* (Windows only): enable ASLR and DEP for globalboost-qt.exe
* (Windows only): add meta-data to globalboost-qt.exe (e.g. description)
...
* Bash programmable completion for globalboostd(1)
